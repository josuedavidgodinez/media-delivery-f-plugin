...
function generateInvoicePDF($BrandName, $Name, $BankAccount, $ClientsName, $EventType, $Service, $EventDate, $BookingHours, $AudioFee, $TravelFee, $Total, $rawMedia, $specificShots, $specialRequests, $videographerSong, $requestedAudio, $experience, $comments, $startEndTimes)
{
    $resp = new stdClass();
    try {
        ...
        $pdf->Cell(35, 10, 'Event Date', 1, 0, 'C', 1);
        $pdf->Cell(50, 10, 'Booking/Event Hours', 1, 0, 'C', 1);
        $pdf->Cell(50, 10, 'Audio Recording Fee (Video)', 1, 0, 'C', 1);
        $pdf->Cell(35, 10, 'Travel Fee', 1, 0, 'C', 1);
        $pdf->Cell(45, 10, 'Total Amount Due', 1, 1, 'C', 1);
        ...
        $pdf->Cell(35, 10, ' ' . $EventDate, 1, 0, 'L');
        $pdf->Cell(50, 10, ' ' . $BookingHours, 1, 0, 'L');
        $pdf->Cell(50, 10, ' $' . $AudioFee, 1, 0, 'L');
        $pdf->Cell(35, 10, ' $' . $TravelFee, 1, 0, 'L');
        $pdf->Cell(45, 10, ' $' . $Total, 1, 1, 'L');


        // Agregar campos faltantes al PDF
        $pdf->SetFont('helvetica', '', 10);
        $pdf->Cell(285, 20, '', 0, 1, 'L');
        $pdf->MultiCell(120, 20, 'Specific Shots or Footage: ' . $specificShots, 0, 'J', false, 0);
        $pdf->Cell(10, 20, '', 0, 0, 'L');
        $pdf->MultiCell(120, 20, 'Special Requests or Notes: ' . $specialRequests, 0, 'J', false, 1);
        $pdf->MultiCell(120, 20, 'Experience: ' . $experience, 0, 'J', false, 0);
        $pdf->Cell(10, 20, '', 0, 0, 'L');
        $pdf->MultiCell(120, 20, 'Comments: ' . $comments, 0, 'J', 1);
